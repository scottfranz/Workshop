...
function formatLine(line: string): string {
  return line
    ...
    .replace(/`(.+?)`/g, "<code style=\"background:rgba(0,0,0,0.06);padding:1px 5px;border-radius:3px;font-size:0.9em;font-family:monospace\">$1</code>");
}
...
function renderMarkdown(text: string): string {
  ...
  for (const line of lines) {
    if (line.startsWith("- ")) {
      ...
      html += "<li>" + formatLine(line.slice(2)) + "</li>";
    } else {
      if (inList) { html += "</ul>"; inList = false; }
      html += (html ? "<br/>" : "") + formatLine(line);
    }
  }
  if (inList) html += "</ul>";
  return html;
...
export default function WritingPage() {
  ...
  function insertBullet() {
    ...
    const end = ta.selectionEnd;
    if (start === end) {
      // No selection: find start of current line and prepend "- "
      const lineStart = body.lastIndexOf("\n", start - 1) + 1;
      const newBody = body.substring(0, lineStart) + "- " + body.substring(lineStart);
      setBody(newBody);
      setTimeout(() => { ta.focus(); ta.setSelectionRange(start + 2, start + 2); }, 0);
    } else {
      // Selection: prefix every line in the selection with "- "
      const lineStart = body.lastIndexOf("\n", start - 1) + 1;
      const chunk = body.substring(lineStart, end);
      const bulleted = chunk.split("\n").map(line => line.startsWith("- ") ? line.slice(2) : "- " + line).join("\n");
      const newBody = body.substring(0, lineStart) + bulleted + body.substring(end);
      setBody(newBody);
      setTimeout(() => { ta.focus(); ta.setSelectionRange(lineStart, lineStart + bulleted.length); }, 0);
    }
  }
  ...
  if (view === "write") return (
    <div style={{ display:"flex", flexDirection:"column", height:"100vh" }}>
      <style>{`
        ...
        .fmt-btn { padding: 4px 9px; border: 1px solid var(--border); border-radius: 4px; background: transparent; cursor: pointer; font-size: 13px; color: var(--ink-faint); font-family: 'Lato', sans-serif; line-height: 1; transition: background 0.1s, color 0.1s; }
        .fmt-btn:hover { background: var(--cream); color: var(--ink); }
      ...
      <div style={{ flex:1, overflowY:"auto", padding:"32px clamp(16px, 5vw, 48px) 48px", display:"flex", flexDirection:"column" }}>
        ...
        <div style={{ display:"flex", gap:4, marginBottom:16, flexWrap:"wrap" }}>
          <button className="fmt-btn" onMouseDown={e => { e.preventDefault(); wrapSelection("**","**"); }} title="Bold (Ctrl+B)"><strong>B</strong></button>
          <button className="fmt-btn" onMouseDown={e => { e.preventDefault(); wrapSelection("*","*"); }} title="Italic (Ctrl+I)" style={{ fontStyle:"italic" }}>I</button>
          <button className="fmt-btn" onMouseDown={e => { e.preventDefault(); wrapSelection("~~","~~"); }} title="Strikethrough"><s>S</s></button>
          <button className="fmt-btn" onMouseDown={e => { e.preventDefault(); wrapSelection("`","`"); }} title="Inline code" style={{ fontFamily:"monospace", fontSize:12 }}>`code`</button>
          <button className="fmt-btn" onMouseDown={e => { e.preventDefault(); insertBullet(); }} title="Bullet list">• List</button>
        </div>
        <textarea ref={textareaRef} value={body} onChange={e => setBody(e.target.value)} placeholder="Start writing…" style={{ flex:1, minHeight:320, width:"100%", fontFamily:"'Lato', sans-serif", fontSize:15, lineHeight:1.85, border:"none", outline:"none", background:"transparent", color:"var(--ink-light)", resize:"none" }} />
      </div>
    </div>
  );
    ...
    <div style={{ display:"flex", flexDirection:"column", height:"100vh" }}>
      ...
      <div style={{ flex:1, overflowY:"auto", padding:"36px 48px" }}>
        <div style={{ maxWidth:680 }}>
          ...
          <div style={{ fontSize:15, lineHeight:1.85, color:"var(--ink-light)" }} dangerouslySetInnerHTML={{ __html: renderMarkdown(selected.body) }} />
        </div>
      </div>
    </div>
  );
